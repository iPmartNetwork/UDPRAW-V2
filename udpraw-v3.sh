...
install_udp2raw() {
    ...
    arch=$(uname -m)
    case "$arch" in
      x86_64) binary="udp2raw_amd64" ;;
      aarch64) binary="udp2raw_arm64" ;;
      armv7l) binary="udp2raw_armv7" ;;
      i686|i386) binary="udp2raw_386" ;;
      *) echo -e "${RED}Unsupported architecture: $arch${NC}"; exit 1 ;;
    esac

    echo -e "${YELLOW}Detected architecture: ${CYAN}$arch${NC}"
    echo -e "${YELLOW}Downloading binary: ${CYAN}$binary${NC}"

    curl -sSL -o "$UDP2RAW_BIN" "https://github.com/iPmartNetwork/UDPRAW-V2/releases/latest/download/$binary"
    chmod +x "$UDP2RAW_BIN"
...
[Service]
ExecStart=$UDP2RAW_BIN $role -l 0.0.0.0:${local_port} -r ${remote} -k "$password" --raw-mode $raw_mode --cipher-mode xor-simple -a
Restart=always
...
optimize_network() {
    clear
    echo -e "${CYAN}Optimizing Network Settings for Lower Latency...${NC}\n"
    echo "Applying TCP BBR, Fast Open, Buffer Enhancements..."
...
while true; do
    ...
    echo -e "${BLUE}5)${NC} Optimize Network for Low Latency"
    echo -e "${BLUE}0)${NC} Exit\n"
    read -rp "Select option [0-5]: " menu_choice
