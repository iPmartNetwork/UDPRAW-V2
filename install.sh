...
menu_status() {
    ...
    for service_file in $(systemctl list-units udp2raw-s-*.service --all --no-legend | awk '{print $1}'); do
        ...
        if systemctl is-active --quiet "${service_file}"; then
            ...
            echo -e "${BG_BLUE}${WHITE} Config (${service_file}) ${NC} > ${RED}Not running. Status: ${status_output}${NC} ${YELLOW}(Check logs with option 4)${NC}"
        fi
    done
    ...
    for service_file in $(systemctl list-units udp2raw-c-*.service --all --no-legend | awk '{print $1}'); do
        c_services_found=1
        if systemctl is-active --quiet "${service_file}"; then
            echo -e "${BG_MAGENTA}${WHITE} Config (${service_file}) ${NC} > ${GREEN}Running.${NC}"
        else
            local status_output=$(systemctl status "${service_file}" | grep "Active:")
            echo -e "${BG_MAGENTA}${WHITE} Config (${service_file}) ${NC} > ${RED}Not running. Status: ${status_output}${NC} ${YELLOW}(Check logs with option 4)${NC}"
        fi
    done
...
while true; do
    ...
    echo -e "${BOLD}${CYAN} 2${NC}) ${GREEN}Set EU Tunnel (New)${NC}"
    echo -e "${BOLD}${CYAN} 3${NC}) ${GREEN}Set IR Tunnel (New)${NC}"  
    echo -e "${BOLD}${CYAN} 4${NC}) ${MAGENTA}Reconfigure an Existing Tunnel${NC}"
    echo -e "${BOLD}${CYAN} 5${NC}) ${BLUE}View Configuration Logs${NC}"
    echo -e "${BOLD}${CYAN} 6${NC}) ${YELLOW}Troubleshoot a Configuration${NC}"
    echo ""
    echo -e "${BOLD}${CYAN} 7${NC}) ${RED}Uninstall UDP2RAW${NC}"
    echo -e "${BOLD}${CYAN} 0${NC}) ${WHITE}Exit${NC}"
    echo ""
    echo -e "${BOLD}${UNDERLINE}${MAGENTA}==========================================${NC}"
    echo ""
    echo -ne "${BOLD}${GREEN}Select an option ${RED}[${WHITE}0-7${RED}]: ${NC}"
    read choice
